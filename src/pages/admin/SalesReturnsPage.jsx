...
function buildSalesReturnExportRows(list, t) {
    const headers = [
        t('th.datetime'), t('th.creditNote'), t('th.invoice'), t('th.customer'), t('th.phone'),
        t('th.vehicle'), t('th.cashier'), t('th.workshop'), t('th.branch'), t('th.type'), t('th.amount'), t('th.status'),
    ];
...
export default function SalesReturnsPage() {
    ...
    const locale =
        ...
        'en';
    const t = useCallback((key, vars) => retT(locale, key, vars), [locale]);
    ...
    }, [selectedWorkshopId, selectedBranchId, statusFilter, dateFrom, dateTo, t]);
    ...
    const runExport = useCallback(async (kind) => {
        ...
        try {
            ...
            const { headers, rows: outRows } = buildSalesReturnExportRows(list, t);
            const subtitle = t('export.subtitle', { n: outRows.length })
                + (dateFrom || dateTo ? ` · ${dateFrom || '…'} → ${dateTo || '…'}` : '')
                + (statusFilter ? ` · ${t('export.status', { status: statusFilter })}` : '');
            if (kind === 'pdf') {
                exportRowsToPdf({ title: t('export.title'), subtitle, headers, rows: outRows, filenameBase: 'sales-returns' });
            } else {
                exportRowsToExcel({ sheetName: t('export.sheet'), headers, rows: outRows, filenameBase: 'sales-returns' });
            }
        } catch (e) {
    ...
    }, [selectedWorkshopId, selectedBranchId, statusFilter, search, dateFrom, dateTo, t]);
        ...
        <div style={{ padding: 20 }}>
            <header style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
                ...
                <div style={{ display: 'flex', gap: 10, alignItems: 'stretch', flexWrap: 'wrap' }}>
                    <div style={{ padding: '8px 14px', borderRadius: 10, background: '#fef2f2', border: '1px solid #fecaca', display: 'flex', gap: 16, alignItems: 'center' }}>
                        <div>
                            <div style={{ fontSize: '0.65rem', color: '#b91c1c', fontWeight: 700, textTransform: 'uppercase' }}>{t('kpi.returns')}</div>
                            <div style={{ fontSize: '0.9375rem', fontWeight: 800, color: '#7f1d1d' }}>{loading ? '—' : Number(kpiCount).toLocaleString()}</div>
                        </div>
                        <div>
                            <div style={{ fontSize: '0.65rem', color: '#b91c1c', fontWeight: 700, textTransform: 'uppercase' }}>{t('kpi.totalReturned')}</div>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 14, alignItems: 'flex-end' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', minWidth: 180 }}>
                    ...
                    <select
                        ...
                        title={selectedWorkshopId ? '' : t('opt.selectWorkshopFirst')}
                    >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', minWidth: 150 }}>
                    <label style={labelStyle}>{t('label.status')}</label>
            ...
            <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 12, overflow: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
                    <thead>
                        <tr>
                            <th style={cellTh}>{t('th.datetime')}</th>
                            <th style={cellTh}>{t('th.creditNote')}</th>
                            <th style={cellTh}>{t('th.invoice')}</th>
                            <th style={cellTh}>{t('th.customer')}</th>
                            <th style={cellTh}>{t('th.vehicle')}</th>
                            <th style={cellTh}>{t('th.cashier')}</th>
                            <th style={cellTh}>{t('th.workshopBranch')}</th>
                            <th style={cellTh}>{t('th.type')}</th>
                            <th style={{ ...cellTh, textAlign: 'right' }}>{t('th.amount')}</th>
                    ...
                    <tbody>
                        {loading ? (
                            <tr><td colSpan={11} style={{ padding: 32, textAlign: 'center', color: '#64748b' }}>
                                <Loader2 size={18} className="spin" /> {t('btn.loading')}
                            </td></tr>
                        ) : rows.length === 0 ? (
                            <tr><td colSpan={11} style={{ padding: 32, textAlign: 'center', color: '#64748b' }}>
                                {t('empty')}
                                    ...
                                    <td style={{ ...cellTd, textAlign: 'right' }}>
                                        <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end', flexWrap: 'wrap' }}>
                                            ...
                                            {r.status === 'pending' && canApprove ? (
                                                ...
                                                >
                                                    {actionLoadingId === `approve-${r.id}` ? '…' : t('btn.approve')}
                                                </button>
                                            ) : null}
                                            ...
                                            {r.invoice?.id ? (
                                                ...
                                                >
                                                    {invoiceLoadingId === String(r.invoice.id) ? <Loader2 size={12} className="spin" /> : <FileText size={12} />} {t('btn.invoice')}
                                                </button>
                                            ) : null}
                                        </div>
                                    </td>
